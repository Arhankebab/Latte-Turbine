...
async function ambilData() {
    ...
    try {
        const response = await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//data');
...
async function loadNotificationSummary() {
    ...
    try {
        const response = await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications/summary');
...
async function checkForNewNotifications() {
    ...
    try {
        const response = await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications/latest');
...
async function populateNotificationDropdown() {
    ...
    if(!container) return;
    try {
        const response = await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications');
...
async function clearNotifications() {
    ...
    try {
        await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications', { method: 'DELETE' });
        loadNotifications();
...
async function checkUnreadCount() {
    ...
    try {
        const response = await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications/unread-count');
        const data = await response.json();
        if (data.count > 0) {
...
async function markNotificationsAsRead() {
    try {
        await fetchDataWithAuth('http://https://latte-turbine-production.up.railway.app//notifications/mark-as-read', { method: 'POST' });
        checkUnreadCount();
...
async function updateDashboardWithHistory(startDateISO, endDateISO) {
    ...
    try {
        ...
        const response = await fetchDataWithAuth(`http://https://latte-turbine-production.up.railway.app//data/history?startDate=${startDateISO}&endDate=${endDateISO}`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://https://latte-turbine-production.up.railway.app//api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, password, registrationCode }) });
                const data = await response.json();
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://https://latte-turbine-production.up.railway.app//api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, password }) });
                const data = await response.json();
                if (!response.ok) throw new Error(data.message);
    ...
    if (downloadPdfButton) {
        downloadPdfButton.addEventListener('click', () => {
            ...
            if (startDate && startHour && endDate && endHour) {
                ...
                const downloadUrl = `http://https://latte-turbine-production.up.railway.app//download-pdf?startDate=${startISO}&endDate=${endISO}&token=${token}&columns=${visibleColumns.join(',')}`;
    ...
    if (berandaFilter) {
        berandaFilter.addEventListener('change', async (e) => {
            ...
            try {
                const response = await fetchDataWithAuth(`http://https://latte-turbine-production.up.railway.app/${endpoint}`);
                const data = await response.json();
                const totalPln = data.totalPlnKwh || 0;
                const totalTurbin = data.totalTurbineKwh || 0;
                if (UIElements.plnEnergyBeranda) UIElements.plnEnergyBeranda.textContent = totalPln.toFixed(2);
